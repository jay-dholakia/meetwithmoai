// Script to create 10 test profiles with v4 questionnaire responses
// Run with: node scripts/create-test-profiles.js
...
if (!supabaseKey || !openaiKey) {
  console.error('Missing SUPABASE_SERVICE_ROLE_KEY or EXPO_PUBLIC_OPENAI_API_KEY');
  process.exit(1);
}
...
// 10 diverse test profiles with realistic responses
const testProfiles = [
  {
...
async function createProfile(profileData) {
  ...
  console.log(`Generating embedding for ${profileData.first_name}...`);
...
async function main() {
  console.log('Creating 10 test profiles with v4 questionnaire responses...\n');
  
  const userIds = [];
  
  for (const profile of testProfiles) {
    const userId = await createProfile(profile);
    if (userId) {
      userIds.push(userId);
    }
  ...
  console.log(`\n✅ Created ${userIds.length} profiles successfully!`);
  console.log(`\nUser IDs:`, userIds);
  
  // Now trigger matching for the first user to show how it works
  console.log(`\n\nRunning matching algorithm for ${testProfiles[0].first_name}...`);
  const response = await fetch(`${supabaseUrl}/functions/v1/replenish-matches`, {
    ...
    body: JSON.stringify({ user_id: userIds[0] }),
  });
  ...
  if (userIds[0]) {
    ...
    console.log('\n\n📊 Top 5 Matches for Sarah Chen:');
    matches.forEach((match, i) => {
      const otherUser = match.user_a === userIds[0] ? match.user_b_profile : match.user_a_profile;
      console.log(`\n${i + 1}. ${otherUser.first_name} ${otherUser.last_name} (Age ${otherUser.age})`);
      console.log(`   Score: ${(match.score * 100).toFixed(1)}%`);
      console.log(`   Reasons:`, JSON.stringify(match.reasons, null, 2));
    });
  }
}
