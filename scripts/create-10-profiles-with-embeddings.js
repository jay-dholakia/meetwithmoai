// Create 10 test profiles with v4 questionnaire and embeddings
...
// Use anon key for reading, but we'll need service role for creating users
// For now, we'll create via SQL using MCP
...
// Profile data with full questionnaire responses
const profiles = [
  {
...
// This will output SQL that can be run via MCP
async function generateSQLForProfiles() {
  console.log('-- SQL to create 10 test profiles with v4 questionnaire\n');
  console.log('-- Note: This requires OpenAI embeddings to be generated separately\n');
  
  for (let i = 0; i < profiles.length; i++) {
    const p = profiles[i];
    const userId = `'${require('crypto').randomUUID()}'`;
    
    console.log(`\n-- Profile ${i + 1}: ${p.first_name} ${p.last_name}`);
    console.log(`-- User ID: ${userId}`);
    console.log(`-- Email: ${p.email}`);
  }
}

// Main function - creates profiles via MCP SQL
async function main() {
  console.log('Creating 10 test profiles...\n');
  console.log('This script will create profiles via MCP SQL.\n');
  console.log('Profiles to create:');
  profiles.forEach((p, i) => {
    console.log(`${i + 1}. ${p.first_name} ${p.last_name} (${p.age}, ${p.city})`);
  });
  
  // We'll create these via MCP SQL calls
  console.log('\n\nCreating profiles via MCP...');
}
...
module.exports = { profiles, generateEmbedding, getQuestionText };
