...
async function generateEmbeddingForUser(userId, userName) {
  ...
  console.log(`   Generating embedding for ${userName}...`);
...
async function main() {
  ...
  for (const intake of allIntakes || []) {
    const profile = intake.profiles;
    const userName = profile ? `${profile.first_name} ${profile.last_name}` : intake.user_id;
    ...
    console.log(`\n📝 Processing ${userName}...`);
    await generateEmbeddingForUser(intake.user_id, userName);
  ...
  if (matches && matches.length > 0) {
    ...
    console.log('   No matches generated yet. The algorithm may need more users or different criteria.');
  }
}
